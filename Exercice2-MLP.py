...
print("CWD :", Path.cwd())
print("ROOT:", ROOT)
print("TRAIN_TSV exists?", TRAIN_TSV.exists())
print("TEST_TSV  exists?", TEST_TSV.exists())
...
# -------------------------- Train/val split (no images loaded) --------------------------
sss = StratifiedShuffleSplit(n_splits=1, test_size=0.10, random_state=seed)
# We don't need features for stratification, pass dummy X
...
res_df.to_csv(ROOT / "results" / "grid_results.csv", index=False)
...
# -------------------------- TEST: read & materialize now, then evaluate --------------------------
