...
Several limitations help explain these findings. First, the sliding-window features are intentionally simple (ink density, vertical profiles, transitions, and basic contour statistics), which captures general stroke structure but not fine-grained letter shapes. In a single-writer setting, many different words share very similar global appearance under such features. Second, the retrieval task is highly imbalanced: each query has only a few true matches among thousands of candidates. A model trained on a balanced pair dataset may still rank many negatives above the positives in this setting. Finally, we did not exploit more advanced preprocessing (e.g. sophisticated binarisation) or more expressive models, in order to stay within the scope of the assignment.

Overall, the project demonstrates that a straightforward DTW-based keyword spotting pipeline can work reasonably well as a baseline, while a simple logistic regression on DTW-derived features does not automatically improve retrieval quality. To obtain substantial gains, future work could explore richer feature sets (e.g. more detailed contour/gradient descriptors), harder negative mining for the learned model, or more powerful similarity learners, while still respecting the general DTW-based framework introduced in the course. From an exercise perspective, the implemented system covers the full pipeline from data loading and word cropping to feature extraction, DTW, learned similarity, and retrieval evaluation, and provides a concrete illustration of both the strengths and limitations of DTW-based keyword spotting on historical handwriting.
